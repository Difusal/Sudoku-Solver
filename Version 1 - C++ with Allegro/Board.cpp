...
Board::Board(void) {
	...
	BoardX = BoardY = (Control::GetInstance()->ScreenHeight - BoardHeight)
			/ 2.0;
...
void Board::Update() {
	...
	if (Control::GetInstance()->Mouse->leftMouseButtonReleased) {
		...
		if (BoardX < Control::GetInstance()->Mouse->x
				&& Control::GetInstance()->Mouse->x < BoardX + BoardWidth
				&& BoardY < Control::GetInstance()->Mouse->y
				&& Control::GetInstance()->Mouse->y < BoardY + BoardHeight) {
			// assigning coordinates of selected cell based on mouse click
			SelectedCellX = BoardX
					+ ((Control::GetInstance()->Mouse->x - BoardX) / CellSize)
							* CellSize;
			SelectedCellY = BoardY
					+ ((Control::GetInstance()->Mouse->y - BoardY) / CellSize)
							* CellSize;
		}
...
void Board::Draw() {
	// white background
	al_draw_filled_rectangle(BoardX, BoardY, BoardX + BoardWidth,
			BoardY + BoardHeight, White);

	// if any cell is selected, draw different background
	if (SelectedCellX != 0 && SelectedCellY != 0)
		al_draw_filled_rectangle(SelectedCellX, SelectedCellY,
				SelectedCellX + CellSize, SelectedCellY + CellSize, Yellow);

	// complementary guide lines
	for (unsigned int i = 1; i <= 8; i++) {
		// horizontal guides
		al_draw_line(BoardX, i * CellSize + BoardY, BoardX + BoardWidth,
				i * CellSize + BoardY, Gray, 1.0);

		// vertical guides
		al_draw_line(i * CellSize + BoardX, BoardY, i * CellSize + BoardX,
				BoardY + BoardHeight, Gray, 1.0);
	}

	// main guide lines
	for (unsigned int i = 1; i <= 3; i++) {
		// horizontal guides
		al_draw_line(BoardX, i * 3 * CellSize + BoardY, BoardX + BoardWidth,
				i * 3 * CellSize + BoardY, Black, 2.0);

		// vertical guides
		al_draw_line(i * 3 * CellSize + BoardX, BoardY,
				i * 3 * CellSize + BoardX, BoardY + BoardHeight, Black, 2.0);
	}

	// limits
	al_draw_rectangle(BoardX, BoardY, BoardX + BoardWidth, BoardY + BoardHeight,
			Black, 2.0);
		...
		for (unsigned int j = 0; j < 9; j++) {
			if (Puzzle[i][j] != 0) {
				...
				al_draw_text(font, Black,
						BoardX + j * CellSize + CellSize / 2.0,
						BoardY + i * CellSize + CellSize / 4.0,
						ALLEGRO_ALIGN_CENTER, ss.str().c_str());
			}
		}
	}
}
