...
Button::Button(string Label, unsigned int CenterX, unsigned int CenterY,
		unsigned int Width, unsigned int Height, unsigned int FontSize) {
...
void Button::Draw() {
	...
	al_draw_rectangle(x + 3, y + 3, x + width - 3, y + height - 3, DarkGray,
			1.0);
	al_draw_rectangle(x + 2, y + 2, x + width - 2, y + height - 2, Gray, 1.0);
	al_draw_rectangle(x + 1, y + 1, x + width - 1, y + height - 1, LightGray,
			1.0);
	al_draw_rectangle(x, y, x + width, y + height, Black, 1.0);

	// printing label
	al_draw_text(font, White, x + width / 2.0 + 1,
			y + 5 + 0.1 * font->height + 1, ALLEGRO_ALIGN_CENTER,
			label.c_str());
	al_draw_text(font, Black, x + width / 2.0, y + 5 + 0.1 * font->height,
