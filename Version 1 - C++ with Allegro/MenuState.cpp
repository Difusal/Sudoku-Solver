...
void MenuState::Initialize() {
	...
	titleFont = al_load_font(CalibriTTF,
			Control::GetInstance()->ScreenHeight / 8.5, ALLEGRO_ALIGN_CENTER);

	// initializing buttons
	solvePuzzleButton = new Button("Solve Puzzle", buttonsX, solvePuzzleY,
			buttonsWidth, buttonsHeight);
	buttons.push_back(solvePuzzleButton);

	fullScreenButton = new Button("Fullscreen", buttonsX, fullScreenY,
			buttonsWidth, buttonsHeight);
	buttons.push_back(fullScreenButton);

	exitButton = new Button("Exit", buttonsX, exitY, buttonsWidth,
			buttonsHeight);
...
void MenuState::Draw() {
	// printing title
	al_draw_text(titleFont, Black,
			Control::GetInstance()->ScreenWidth / 2.0 + 3, titleY + 2,
			ALLEGRO_ALIGN_CENTER, "SUDOKU SOLVER");
	al_draw_text(titleFont, White, Control::GetInstance()->ScreenWidth / 2.0,
			titleY, ALLEGRO_ALIGN_CENTER, "SUDOKU SOLVER");
	...
	al_draw_text(Control::GetInstance()->mediumFont, Black,
			Control::GetInstance()->ScreenWidth
					- Control::GetInstance()->mediumFont->height / 2 + 1,
			Control::GetInstance()->ScreenHeight
					- Control::GetInstance()->mediumFont->height + 1,
			ALLEGRO_ALIGN_RIGHT, "Henrique Ferrolho - 2013");
	al_draw_text(Control::GetInstance()->mediumFont, White,
			Control::GetInstance()->ScreenWidth
					- Control::GetInstance()->mediumFont->height / 2,
			Control::GetInstance()->ScreenHeight
					- Control::GetInstance()->mediumFont->height,
			ALLEGRO_ALIGN_RIGHT, "Henrique Ferrolho - 2013");
}
